...
def process_value_to_richtext(val):
    ...
    if is_number:
        # 這裡已經建立好紅字物件了，所以 Word 裡只要用 {{ 變數 }} 即可
        rt = RichText()
...
1. **單一變數（例如：COP、效率、kWh 等）** - 請放在 Excel 的 `變數` 或 `Variables` 工作表中。  
   - 第 1 欄為「變數名稱」，第 2 欄為「數值」，其餘欄位會被忽略。  
   - **修正說明**：在 Word 中直接使用：`{{ 變數名稱 }}` 即可（程式會自動變紅）。
...
3. **關於紅字 (RichText)** - 本程式會自動偵測「數字」，並將其轉為**紅字粗體**。
   - Word 模板中 **請勿** 加 `r`，直接寫 `{{ 變數 }}` 即可顯示紅字。
""")
...
if uploaded_word and uploaded_excel:
    ...
    if st.button("🚀 開始生成報告", type="primary"):
        try:
            ...
            # 檢查是否為字串 (因為可能是 RichText 物件，需轉回字串或是避開)
            if hasattr(file_name_var, 'xml'): # 如果它是 RichText，通常不適合當檔名
                 download_name = "報告_表格連動版.docx"
